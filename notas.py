        ...
        try:
            with open(ARQUIVO, "r", encoding="utf-8") as f:
                ...
                if isinstance(dados, list):
                    ...
                    print("Formato de arquivo invalido. Iniciando a lista vazia.")
                    return[]
...
def deletar_nota():
    ...
    try:
        indice = int(input("Digite o número da nota a ser excluído: "))
        if 1 <= indice < len(notas):
            nota_removida = notas.pop(indice - 1)
            salvar_notas(notas)
            print(f"Nota '{nota_removida.get('titulo', 'semtitulo')}' excluída com sucesso.")
        else:
            print("Nota inválida.")
    except ValueError:
        print("Digite apenas um número válido.")
